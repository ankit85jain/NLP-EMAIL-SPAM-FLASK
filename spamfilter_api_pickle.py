...
def train_dataset():
    ...
    if request.method == 'POST':
        ...
        modelpath = os.path.join(current_app.config['ML_MODEL_UPLOAD_FOLDER'],"model_class.pk")
        print(modelpath)
        filehandler = open(modelpath,"rb")
        model_class = pickle.load(filehandler)
        filehandler.close()
        classifier, word_features = model_class.train(df[:,0:1],df[:,1])
    ...
    if request.method == 'POST':
        ...
        if valid == False:
          ...
          if not inputmodel:
            ...
            modelpath = os.path.join(os.path.dirname(current_app.config['BASE_DIR']),"model_class.pk")
            filehandler = open(modelpath,"rb")
            spamclassifier = pickle.load(filehandler)
            filehandler.close()
